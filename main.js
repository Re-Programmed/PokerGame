...
function tryMenuMusic()
{
    ...
    if(currentSettings.enablemenumusic == undefined ? false : currentSettings.enablemenumusic)
    {
        var randMusic = pageMusicModifier < 0 ? Math.floor(Math.random() * 7) : pageMusicModifier;
        switch(randMusic)
        {
            case 0:
                PlaySound(SOUNDS.Music_Chill_Chords, true);
                break;
            case 1:
                PlaySound(SOUNDS.Music_MainTheme, true);
                break;
            case 2:
                PlaySound(SOUNDS.Music_Chill_Drums, true);
                break;
            case 3:
                PlaySound(SOUNDS.Music_Chill_AllPiano, true);
                break;
            case 4:
                PlaySound(SOUNDS.Music_Chill_Chords, true);
                break;
            case 5:
                PlaySound(SOUNDS.Music_Chill_Melody, true);
                break;
            case 6:
                PlaySound(SOUNDS.Music_Chill_NoDrums, true);
                break;
        }
    }
}
...
window.addEventListener('load', function () {
    ...
    this.setTimeout(tryMenuMusic, 1500);
...
function CreateSearchMenu(term, player = false, table = false)
{
    ...
    if(Array.from(document.getElementsByClassName("search_menu")).length == 0)
    {
    ...
    if(table)
    {
        rc_0x01623.rc_0x01623_gr(term).then(table => {
            if(table == null)
            {
    ...
    if(player)
    {
        rc_0x01623.rc_0x01623_gu(term).then(user => {
            if(user == null)
            {
    ...
    if(!player && !table)
    {
        $(menu).append("<p onclick='CreateSearchMenu(null, true);PlaySound(SOUND.SwitchButton);'>Search for player...</p>")
        $(menu).append("<p onclick='CreateSearchMenu(null, false, true);PlaySound(SOUND.SwitchButton);'>Search for table...</p>")
    }
