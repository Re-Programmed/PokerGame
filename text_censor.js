...
function censorText(text, useRed = false)
{
    const arr = CENSOR_regex.split("\n");
    for(var i = 0; i < arr.length; i++)
    {
        text = text.replace(new RegExp(arr[i]), m => (useRed ? `<span style="color: orangered;">*</span>` : "*"));
    }
...
function FileAReport(templateReason = "")
{
    ...
    parent.innerHTML = REPORT_SCREEN.replace(/%tempateReason%/g, templateReason == "" ? randomDefaultReports[Math.floor(Math.random() * randomDefaultReports.length)] : templateReason).replace(/%playerName%/g, pthe_0x155_cU.username);
    ...
    setTimeout(() => {
        ...
        setInterval(() => {
            ...
            $("#report_player").val(pthe_0x155_cU.username);
...
function SubmitReport()
{
    ...
    if(localStorage.getItem("pokerGAME_provided_reports") != null && JSON.parse(localStorage.getItem("pokerGAME_provided_reports")).includes(pthe_0x155_cU.username))
    {
    ...
    rc_0x01623.rc_0x01623_gi("content_reports").then(reports => {
        var arr = JSON.parse(atob(reports.data));
        arr.push({r: reasond.value, s: saymored.value, p: pthe_0x155_cU.username});
        ...
        localReports.push(pthe_0x155_cU.username);

        localStorage.setItem("pokerGAME_provided_reports", JSON.stringify(localReports));

        rc_0x01623.rc_0x01623_si("content_reports", btoa(JSON.stringify(arr))).then(d => {
