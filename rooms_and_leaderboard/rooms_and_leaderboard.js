...
window.addEventListener('load', function () {
    ...
    SetBackgroundScroll(0.25);
     ...
     API.GetUser(acc.username).then(user => {
        ...
        if(currentUser.ll_check == undefined || Date.now() - currentUser.ll_check > 86400000)
        {
       ...
       UpdateThemeOnLoad(currentUser);
        ...
        API.GetLeaderboard().then(leaderboard_data => {
            leaderboardData = leaderboard_data;
            var leaderboard = leaderboardData.board;

            //Determine if the user should be added to the board.
            const addResult = UpdateUserLeaderboardSlot(leaderboard, {username: currentUser.username, s: currentUser.money, g: 0, c: currentUser.customize});
            ...
            if(addResult != null)
            {
                ...
                API.SetLeaderboard({board: addResult, update: leaderboardData.update});
                return;
            }
        ...
        if(user.l_board != null && user.l_board != undefined && user.l_board != "")
        {
            if(user.l_board.startsWith("REQUEST_"))
            {
                ...
                if(GetSetting(user.username, "allowleaderboardinvites", true) == true)
                {
                ...
                API.GetItem("local_leaderboard_" + user.l_board).then(localBoard => {
                    currentLocalBoard = localBoard;
                    const board_data = JSON.parse(atob(currentLocalBoard.data));
                    const updateLocal = UpdateUserLeaderboardSlot(board_data, {username: currentUser.username, s: currentUser.money, g: 0, c: currentUser.customize});
                    console.log(updateLocal);

                    if(updateLocal != null)
                    {
                        API.SendItem("local_leaderboard_" + currentUser.l_board, btoa(JSON.stringify(updateLocal)));
...
{
    CreateConfirmationMenu("Are you sure you want to leave " + currentUser.l_board + "?", () => {
        $(".local_option>button").text("Loading...");   
        API.GetItem("local_leaderboard_" + currentUser.l_board).then(lBoard => {
            //Remove the user from the local board.
            var board_data = JSON.parse(atob(currentLocalBoard.data));
            for(var i = 0; i < board_data.length; i++)
            {
                if(board_data[i].username == currentUser.username)
                {
            ...
            API.SendItem("local_leaderboard_" + currentUser.l_board, btoa(JSON.stringify(board_data))).then(d => {
...
function CreateRoomGUI(code, size, data = null)
{
    ...
    */
    ...
    room.innerHTML = room_HTML.replace(/%code%/g, code).replace(/%size%/g, `${size}/4`).replace(/%stake%/g, stake).replace(/%stakeColor%/g, stakeColor).replace(/%minBet%/g, data.MinimumBet);
...
function CreateLeaderboardSlot(username, score, games_played, customize = null, global = true, forceCrown = false)
{
    ...
    if(username != "Name")
    {
        ...
        lb.addEventListener("click", function () {
            if(currentUser.username != this.getAttribute("username"))
            {
    ...
    var html = leaderboard_HTML.replace(/%name%/g, username).replace(/%sc%/g, `$${score}`).replace(/%pl%/g, pl < 0 ? "#" : ++pl).replace(/%g%/g, games_played);
...
function FinishCreate()
{
    ...
    API.GetAllRooms().then(rooms => {
        ...
        var packs = currentUser.customize.Pack;
        ...
        const json = `{"c_time":"${btoa(new Date().toLocaleString())}","players":[],"code":"","game":"${gType}","theme":"${tTheme}","CurrentTurn":1,"CurrentBlind":1,"CurrentBet":${minBet},"CurrentStage":0,"RoundsPlayed":0,"QueuedBets":[],"AllIn":[],"Creator":"${currentUser.username}","MinimumBet":${minBet},"PlayerCards":[{"c1":null,"c2":null},{"c1":null,"c2":null},{"c1":null,"c2":null},{"c1":null,"c2":null}],"Folded":[],"Pot":0,"PlayerBets":[0,0,0,0],"CheckingBets":[],"PublicCards":["","","","",""],"ShowingCards":-1,"Winners":[],"Waiting":true,"Votes":[],"G_Fun":{"x":0,"y":0,"p":0},"watching":[],"stake":"${stake}","custom":{"Set":[${currentUser.customize.Set}],"Pack":[${packs}]}}`;
        console.log(json);
...
function CreateRoom()
{
    ...
    if(currentUser.money > 5000)
    {
        $("#tbl_stake>option:first-child").remove();
    }   
    if(currentUser.money > 10000)
    {
        $("#tbl_stake>option:first-child").remove();
    }  
    if(currentUser.money > 15000)
    {
        $("#tbl_stake>option:first-child").remove();
    }  
    if(currentUser.money > 20000)
    {
        $("#tbl_stake>option:first-child").remove();
    }  
    if(currentUser.money > 30000)
    {
        $("#tbl_stake>option:first-child").remove();
    }  
...
function AdvanceNews(amount = 1)
{
    ...
    {
        if(currentUser.l_board.startsWith("REQUEST_"))
        {
            currentUser.l_board = "";
            API.UpdateUser(currentUser);
        }
...
function CreateNews_LLInvite(inviteName)
{
    ...
    acceptButton.addEventListener('click', function () {
        ...
        currentUser.l_board = currentUser.l_board.replace("REQUEST_", "");
        API.UpdateUser(currentUser).then(d => {
            alert('You have joined the local board \"' + currentUser.l_board + "\"");
