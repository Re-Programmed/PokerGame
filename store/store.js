...
{
    API.GetItem("content_store").then(store => {
        ...
        GetProducts(storeData.daily).forEach(product => {
            ...
            {
                if(MAIN_CATALOG[currentUser.c_owned[i]].Name == product.Name)
                {
...
const PermanentStoreItems = [0,1,2,3,
    5, //Black Chip Border
    6, //Blue Chip Border
    8, //Green Chip Border
    11, //Pink Chip Border
...
function LoadPermanentItems()
{
    ...
    for(var i = 0; i < PermanentStoreItems.length; i++)
    {
     ...
     //Check if the item is onwed by the user.
     for(var j = 0; j < currentUser.c_owned.length; j++)
    {
        console.log(currentUser.c_owned[j])
        if(!PermanentStoreItems.includes(currentUser.c_owned[j])){continue;}

        //Item is already owned by the user!
        prod[PermanentStoreItems.indexOf(currentUser.c_owned[j])].SetOwned();
    }
}
        ...
        setTimeout(function () {
            ...
            for(var i = 0; i < MAIN_PACK_CATALOG.length; i++)
            {
                ...
                if(MAIN_PACK_CATALOG[i].name == prodDisplay.getAttribute("item_name"))
                {
                    ...
                    while(currentUser.c_owned.includes(resultingItemID) && packRetries < 100)
                    {
                    ...
                    currentUser.money -= parseInt(prodDisplay.getAttribute("item_cost"));
                    $("#user_dollar_count").text("$" + currentUser.money);
                    
                    //Give the user the item only if they do not already have it.
                    if(!currentUser.c_owned.includes(resultingItemID))
                    {
                        currentUser.c_owned.push(resultingItemID);
                    }
...
function BuyItemAnimation(prodDisplay, bypass66Check = false)
{
    ...
    if(parseInt(prodDisplay.getAttribute("item_cost")) > currentUser.money)
    {
        return;
    }

    if(!bypass66Check && parseInt(prodDisplay.getAttribute("item_cost")) > currentUser.money * 0.66)
    {
                                        ...
                                        setTimeout(function () {
                                            ...
                                            for(var i = 0; i < MAIN_CATALOG.length; i++)
                                            {
                                                if(prodDisplay.getAttribute("item_name") == MAIN_CATALOG[i].Name)
                                                {
                                                    ...
                                                    currentUser.money -= MAIN_CATALOG[i].GetTotalPrice();
                                                    $("#user_dollar_count").text("$" + currentUser.money);
                                                    currentUser.c_owned.push(i);
                                                    API.UpdateUser(currentUser);
                                                    break;
                                                }
                                            }
...
function CreateNewStore(dItems = null)
{
    ...
    API.SendItem("content_store", btoa(newStore)).then(d => {
