...
function RenderCard(cardParent, card, scale = 1)
{
    ...
    if(pthe_0x155_cU != undefined)
    {
        ...
        {
            if(pthe_0x155_cU.customize.Pack.length > 1)
            {
                for(var i = 0; i < pthe_0x155_cU.customize.Pack.length; i++)
                {
                    if(pthe_0x155_cU.customize.Pack[i] < 0){continue;}
                    if(MAIN_CATALOG[pthe_0x155_cU.customize.Pack[i]].Data.replaces.includes(card.GetImage().replace(".png", "")))
                    {
                        console.log("../assets/cards/" + MAIN_CATALOG[pthe_0x155_cU.customize.Pack[i]].Data.value + "/" + card.GetImage());
                        cardobj.src = "../assets/cards/" + MAIN_CATALOG[pthe_0x155_cU.customize.Pack[i]].Data.value + "/" + card.GetImage();
                        found = true;
                        break;
                    }
                }
