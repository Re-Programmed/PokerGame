...
window.addEventListener('load', function () {
    pageMusicModifier = 1; //Force Main Theme.
    ...
    rc_0x01623.rc_0x01623_gu(viewingAccount == "" ? acc.username : viewingAccount).then(user => {
        //Does the user exist?
        ...
        if(user.customize != null && viewingAccount == "")
        {
            ...
            this.setTimeout(() => {
                ...
                ApplyChipSetDisplay(pthe_0x155_cU.customize.Set);

                LoadCardPackApplicator();
                ApplyCardPackDisplay(pthe_0x155_cU.customize.Pack);
            }, 40)
        ...
        if(viewingAccount != "")
        {
            ...
            document.getElementById("user_username").style.fontFamily = `"${pthe_0x155_cU.customize.Font}", "Merriweather", serif`
            document.getElementById("user_username").style.color = pthe_0x155_cU.customize.NameColor.startsWith("#") ? pthe_0x155_cU.customize.NameColor : "#" + pthe_0x155_cU.customize.NameColor;
            document.getElementById("user_icon_border").style.backgroundImage = "url('../assets/user_borders/" + pthe_0x155_cU.customize.Border + ".png')";

            this.document.getElementById("account_display").style.backgroundColor = pthe_0x155_cU.customize.BGColor.startsWith("#") ? pthe_0x155_cU.customize.BGColor : "#" + pthe_0x155_cU.customize.BGColor;
...
function NAME_CustomizeOptions_genericINIT(custom, thizz, updateGraphics = null, validate = null)
{
    ...
    const changeFunc = function (val, inp){
        ...
        {
            pthe_0x155_cU.customize[tUp] = val;
...
{
    const prod = GetProducts(pthe_0x155_cU.c_owned);
...
{
    var moneyLeft = pthe_0x155_cU.money;
...
{
    if(pthe_0x155_cU.customize.Set == undefined)
    {
        pthe_0x155_cU.customize.Set = [-1];
    }

    if(pthe_0x155_cU.customize.Set.includes(itemID))
    {
        return;
    }

    SetChangeWasMade(true);
    pthe_0x155_cU.customize.Set.unshift(itemID);
...
{
    if(pthe_0x155_cU.customize.Pack == undefined)
    {
        pthe_0x155_cU.customize.Pack = [-1];
    }

    if(pthe_0x155_cU.customize.Pack.includes(itemID))
    {
        return;
    }

    SetChangeWasMade(true);
    pthe_0x155_cU.customize.Pack.unshift(itemID);
...
function UnapplyChipSet(itemID)
{
    ...
    SetChangeWasMade(true);
    //SPLICE MUTATES
    pthe_0x155_cU.customize.Set.splice(pthe_0x155_cU.customize.Set.indexOf(itemID), 1);
...
function UnapplyCardPack(itemID)
{
    ...
    SetChangeWasMade(true);
    pthe_0x155_cU.customize.Pack.splice(pthe_0x155_cU.customize.Pack.indexOf(itemID), 1);
...
function LoadCardPackApplicator()
{
    for(var i = 0; i < pthe_0x155_cU.c_owned.length; i++)
    {
        const item = MAIN_CATALOG[pthe_0x155_cU.c_owned[i]];
        if(item.Type == PRODUCT_TYPE.CARD_PACKS)
        {
            var created = null;
            if(pthe_0x155_cU.customize.Pack.includes(pthe_0x155_cU.c_owned[i]))
            {
            ...
            created.setAttribute("my_pack", pthe_0x155_cU.c_owned[i]);
...
function LoadChipSetApplicator()
{
    for(var i = 0; i < pthe_0x155_cU.c_owned.length; i++)
    {
        ...
        {
            var created = null;
            if(pthe_0x155_cU.customize.Set.includes(pthe_0x155_cU.c_owned[i]))
            {
            ...
            created.setAttribute("my_set", pthe_0x155_cU.c_owned[i]);
...
function AddToLocalLeaderboard()
{
    if(viewingAccount != "")
    {
        ...
        rc_0x01623.rc_0x01623_gu(acc.username).then(myUser => {
            ...
            if(myUser.l_board != null && myUser.l_board != undefined && myUser.l_board != "")
            {
                ...
                askToJoin.innerHTML = `
                    ...
                    <h3>Ask ${pthe_0x155_cU.username} To Join ${myUser.l_board}?</h3>
                ...
                setTimeout(() => {
                    document.getElementById("ask_button").addEventListener('click', function () {
                        ...
                        rc_0x01623.rc_0x01623_gu(pthe_0x155_cU.username).then(updatedUser => {
                            ...
                            rc_0x01623.rc_0x01623_uu(updatedUser);
...
function AttemptUnlockCustomizeFeature(feature, cost, featureName = feature)
{
    ...
    if(pthe_0x155_cU.money >= cost && !pthe_0x155_cU.c_unlock.includes(feature))
    {
        //Confirm with the user, explaining how much they will have left.
        CreateConfirmationMenu(`Do you want to unlock ${featureName}? (You will have $${pthe_0x155_cU.money - cost} left)`, () => {
            pthe_0x155_cU.money -= cost;
            pthe_0x155_cU.c_unlock.push(feature);

            //Update and reload.
            rc_0x01623.rc_0x01623_uu(pthe_0x155_cU).then(d => {window.location.reload();});
        });
    }
}